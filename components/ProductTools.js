...
import { AlertTriangle, Bell, Calculator, Camera, Check, CircleDollarSign, Landmark, MapPin, PiggyBank, ReceiptText, ScanLine, WalletCards } from 'lucide-react';
...
export default function ProductTools({ allRates = {}, fromCurrency = 'EUR', toCurrency = 'XOF', currencies = [], lang = 'fr' }) {
  ...
  return <div className="min-w-0"><div className="mb-6"><p className="text-[11px] font-semibold uppercase tracking-[.16em] text-emerald-700 dark:text-emerald-400">Kiwango tools</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.045em] sm:text-4xl">Gérez l’argent du voyage, pas seulement la conversion.</h1></div><div className="grid min-w-0 gap-6 lg:grid-cols-[230px_minmax(0,1fr)]"><aside className="min-w-0"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:sticky lg:top-24 lg:grid-cols-1">{TOOL_ITEMS.map(([id,label,Icon]) => <button key={id} onClick={() => setTool(id)} className={`flex min-w-0 items-center gap-2.5 rounded-2xl px-3 py-3 text-left text-xs font-medium transition ${tool === id ? 'bg-slate-950 text-white dark:bg-white dark:text-slate-950' : 'bg-white text-slate-600 hover:bg-slate-100 dark:bg-white/[.035] dark:text-slate-300 dark:hover:bg-white/[.07]'}`}><Icon className="h-4 w-4 flex-none"/><span className="truncate">{label}</span></button>)}</div></aside><div className="min-w-0">
    ...
    {tool === 'alerts' && <RateAlerts marketRate={marketRate} base={base} quote={quote} setBase={setBase} setQuote={setQuote} options={options} locale={locale}/>} 
...
function SmartCalculator({ marketRate, base, quote, setBase, setQuote, options, locale }) {
 const [expr,setExpr]=useState('450 * 3 + 120'); let result=null; try { if (/^[0-9+\-*/().\s]+$/.test(expr)) result=Function(`"use strict";return (${expr})`)(); } catch {}
...
function RateAlerts({ marketRate, base, quote, setBase, setQuote, options, locale }) {
 ...
 return <Shell title="Alertes de taux" subtitle="Kiwango vérifie ces seuils quand l’application est ouverte. Les notifications en arrière-plan nécessiteront ensuite un service push."><PairFields {...{base,quote,setBase,setQuote,options}}/><div className="mt-4 flex gap-2"><Input value={target} onChange={e=>setTarget(e.target.value)} placeholder={`Alerter quand 1 ${base} ≥ … ${quote}`}/><button onClick={add} className="rounded-2xl bg-emerald-600 px-5 text-sm font-semibold text-white">Créer</button></div><div className="mt-5 space-y-2">{alerts.map(a=>{const current=calculateCrossRate(a.base,a.quote,allSafeRates(), 'EUR'); const hit=a.base===base&&a.quote===quote&&marketRate>=a.target; return <div key={a.id} className="flex items-center justify-between rounded-2xl border border-slate-200 p-4 text-sm dark:border-white/10"><span>1 {a.base} ≥ {fmt(a.target,locale,4)} {a.quote}</span><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${hit?'bg-emerald-50 text-emerald-700':'bg-slate-100 text-slate-500 dark:bg-white/10'}`}>{hit?'Seuil atteint':'En veille'}</span></div>})}</div></Shell>;
 function allSafeRates(){ return {}; }
}
