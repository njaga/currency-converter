...
export default function Logo({ size = 'md', showText = true, variant = 'auto', className = '', href = '/' }) {
  ...
  const content = <div className={`flex min-w-0 items-center gap-2.5 ${className}`}>
    ...
    {showText && <span className={`${textSizes[size]} truncate font-semibold tracking-[-0.045em] ${textColor}`}>Kiwango</span>}
  </div>;

  return <Link href={href} aria-label="Kiwango — accueil" className="inline-flex min-w-0 items-center">{content}</Link>;
}
