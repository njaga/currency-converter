...
export default function TripPreparation({ country, lang = 'fr' }) {
  ...
  const query = new URLSearchParams({ countryCode: country.code, country: country.name, currency, lang }).toString();
  ...
  return <section className="mt-6 min-w-0 rounded-[30px] border border-slate-200/80 bg-slate-950 p-5 text-white shadow-[0_22px_70px_rgba(15,23,42,.14)] sm:p-7 dark:border-white/10">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div className="max-w-2xl"><p className="text-[11px] font-semibold uppercase tracking-[.16em] text-emerald-400">Préparer le départ</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.035em] sm:text-3xl">Tout préparer pour {country.name}, au même endroit.</h2><p className="mt-2 text-sm leading-6 text-slate-400">Ces services s’ouvrent chez des partenaires externes. Lorsqu’un lien est affilié, Kiwango peut recevoir une commission sans coût supplémentaire pour vous.</p></div>
      <span className="text-xs text-slate-500">Liens partenaires · transparence incluse</span>
    </div>
    <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {SERVICES.map(({ id, label, description, icon: Icon }) => <a key={id} href={`/go/${id}?${query}`} target="_blank" rel="sponsored noopener noreferrer" onClick={() => trackEvent('affiliate_click', { service: id, country: country.code, currency })} className="group min-w-0 rounded-[22px] border border-white/10 bg-white/[.055] p-4 transition hover:-translate-y-0.5 hover:border-emerald-400/40 hover:bg-white/[.08]">
