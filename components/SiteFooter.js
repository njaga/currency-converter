...
export default function SiteFooter({ lang }) {
  ...
  const groups = [
    { title: fr ? 'Explorer' : 'Explore', links: [[fr ? 'Convertisseur' : 'Converter','/app?tab=converter'],[fr ? 'Toutes les destinations' : 'All destinations','/app?tab=travel'],[fr ? 'Outils financiers' : 'Money tools','/app?tab=tools'],[fr ? 'Devises' : 'Currencies','/app?tab=rates'],[fr ? 'Télécharger l’application' : 'Get the app','/#download']] },
    { title: fr ? 'Destinations populaires' : 'Popular destinations', links: popular.map(([label,slug]) => [label, `/voyage/${slug}`]) },
    { title: fr ? 'Ressources' : 'Resources', links: [['Rate Check','/app?tab=tools'],[fr ? 'Budget voyage' : 'Travel budget','/app?tab=tools'],['Cash Wallet','/app?tab=tools'],[fr ? 'Mentions légales & confidentialité' : 'Legal & privacy','/mentions-legales']] },
  ];
