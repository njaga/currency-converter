...
async function calculateRoute(origin, destination, lang, key) {
  const params = new URLSearchParams({ origin, destination, language: lang, mode: 'driving', key });
  const response = await fetch(`https://maps.googleapis.com/maps/api/directions/json?${params.toString()}`, { signal: AbortSignal.timeout(9000) });
  if (!response.ok) throw new Error(`Google Directions ${response.status}`);
  const data = await response.json();
  if (data.status !== 'OK' || !data.routes?.[0]?.legs?.[0]) throw new Error(`Google Directions ${data.status || 'NO_ROUTE'}`);
  ...
  return {
    ...
    steps: (leg.steps || []).map((step) => ({ instruction: stripHtml(step.html_instructions), distance: step.distance?.text || '', duration: step.duration?.text || '' })).filter((step) => step.instruction),
  };
}
...
export default async function handler(req, res) {
  ...
  if (!['route','banks','exchange'].includes(type) || !destination || (type === 'route' && !origin)) return res.status(400).json({ error: 'Invalid travel map request' });
  const key = process.env.GOOGLE_MAPS_API_KEY;
  if (!key) return res.status(503).json({ error: 'Google Maps is not configured' });

  try {
    res.setHeader('Cache-Control', 'private, max-age=120');
    if (type === 'route') return res.status(200).json({ type, provider: 'Google Maps', route: await calculateRoute(origin, destination, lang, key) });
