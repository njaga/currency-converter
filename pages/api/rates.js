...
async function currencyApi(base, apiKey) {
  ...
  try {
    ...
    url.searchParams.set('type', 'fiat');
    ...
    const rates = Object.fromEntries(
      Object.entries(data.data)
        .map(([code, item]) => [String(code).toUpperCase(), Number(item?.value)])
    ...
    return {
      ...
      rates,
      providerUpdatedAt: data.meta?.last_updated_at ? Date.parse(data.meta.last_updated_at) : null,
    };
  } catch {
    return null;
  }
...
export default async function handler(req, res) {
  ...
  const providers = [
    () => exchangeRateApi(base, process.env.EXCHANGE_RATE_API_KEY),
    () => currencyApi(base, process.env.CURRENCY_API_KEY),
  ];

  if (!process.env.EXCHANGE_RATE_API_KEY && !process.env.CURRENCY_API_KEY) {
    return res.status(503).json({ error: 'No server fallback rate provider is configured' });
  }
