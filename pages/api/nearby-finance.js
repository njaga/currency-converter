...
function normalizeGoogle(place, fallbackType = 'financial_service') {
  return {
    id: place.id,
    type: place.primaryType || fallbackType,
...
async function searchGoogle(lat, lon, radius, lang) {
  ...
  const [nearby, exchange, transfer] = await Promise.allSettled([
    googlePost('places:searchNearby', {
      includedTypes: ['atm', 'bank'],
      maxResultCount: 20,
      rankPreference: 'DISTANCE',
      languageCode: lang === 'en' ? 'en' : 'fr',
      locationRestriction: { circle },
    }, key),
    googlePost('places:searchText', {
      textQuery: lang === 'en' ? 'currency exchange' : 'bureau de change',
      maxResultCount: 10,
      languageCode: lang === 'en' ? 'en' : 'fr',
      locationBias: { circle },
    }, key),
    googlePost('places:searchText', {
      textQuery: lang === 'en' ? 'money transfer' : 'transfert d’argent',
      maxResultCount: 10,
      languageCode: lang === 'en' ? 'en' : 'fr',
      locationBias: { circle },
    }, key),
  ...
  const merged = [];
  const push = (result, fallbackType) => {
    if (result.status !== 'fulfilled') return;
    for (const place of result.value?.places || []) merged.push(normalizeGoogle(place, fallbackType));
  };
  push(nearby, 'financial_service');
  push(exchange, 'currency_exchange');
  push(transfer, 'money_transfer');

  const unique = Array.from(new Map(merged.filter((item) => item.id).map((item) => [item.id, item])).values());
  return unique.filter((item) => Number.isFinite(item.lat) && Number.isFinite(item.lon));
}
...
export default async function handler(req, res) {
  ...
  const radius = Math.min(5000, Math.max(300, Number(req.query.radius) || 2200));
  const lang = req.query.lang === 'en' ? 'en' : 'fr';
  if (!validCoord(lat, -90, 90) || !validCoord(lon, -180, 180)) return res.status(400).json({ error: 'Invalid coordinates' });

  try {
    const googleItems = await searchGoogle(lat, lon, radius, lang);
    if (googleItems?.length) {
      res.setHeader('Cache-Control', 'private, max-age=120');
      return res.status(200).json({ items: googleItems, radius, provider: 'Google Places' });
    }
  } catch {
    // Fall through to OpenStreetMap so this utility keeps working if Places is unavailable.
  }

  try {
    const items = await searchOpenStreetMap(lat, lon, radius);
    res.setHeader('Cache-Control', 's-maxage=300, stale-while-revalidate=900');
    return res.status(200).json({ items, radius, provider: 'OpenStreetMap / Overpass API', fallback: true });
  } catch {
